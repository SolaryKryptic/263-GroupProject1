...
ALLOW_SHEDDING = True      # the fuel-reduction proposal's core lever. True = the proposal's
...
def report_and_save(day, status, owned_routes, leased_routes, skipped, oc, lc,
                     output_dir=OUTPUT_DIR, allow_shedding=None):
    ...
    with open(path, "w", newline="") as f:
        ...
        writer.writerow(["solver_status", status])
        if allow_shedding is not None:
            writer.writerow(["shedding", "enabled" if allow_shedding else "disabled (baseline)"])
        writer.writerow(["owned_trips", f"{len(owned_routes)}/{MAX_OWNED_TRIPS}"])
        writer.writerow(["leased_trips", len(leased_routes)])
        writer.writerow(["skipped_stores", len(skipped)])
        if skipped:
            writer.writerow(["skipped_store_names", "; ".join(skipped)])
        writer.writerow(["owned_cost", round(total_owned, 2)])
        writer.writerow(["leased_cost", round(total_leased, 2)])
        writer.writerow(["skip_cost", round(total_skip, 2)])
        writer.writerow(["total_cost", round(grand_total, 2)])
