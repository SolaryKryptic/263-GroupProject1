...
WAREHOUSE_COORD = None  # filled in by load_locations()
...
def load_day_data(day):
    ...
    return baseline, shed_stores


def build_map(day, coord_map, baseline, shed_stores, get_route_coords, title_suffix=""):
    ...
    routes_layer = folium.FeatureGroup(name=f'Delivery Routes ({len(baseline)})', show=True)
    for i, row in baseline.iterrows():
        ...
        folium.PolyLine(line_coords, color=color, weight=3.5, opacity=0.8,
                        popup=folium.Popup(popup_text, max_width=300)).add_to(routes_layer)
    ...
    if shed_stores:
        shed_layer = folium.FeatureGroup(name=f'Shed Stores - Fuel Reduction ({len(shed_stores)})', show=False)
    ...
    title_html = f'''
        <div style="position: fixed; top: 10px; left: 60px; z-index: 9999;
            ...
            <span style="font-size:12px; color:#555;">{len(baseline)} routes, {baseline['total_pallets'].sum():.0f} pallets total</span>
        </div>
    '''
