...
OUTPUT_DIR = "."
DEMAND_CSV = "ayush_estimates.csv"
...
def build_model(routes, store_types):
...
def export_lp(day, routes, store_types, output_dir=OUTPUT_DIR):
    """
    ...
    """
    prob, x, v, z, oc, lc = build_model(routes, store_types)
    path = Path(output_dir) / f"{day.lower()}_model.lp"
...
def main():
    ...
    if args.export_lp:
        export_lp(args.day, routes, store_types)
        return

    status, owned_routes, leased_routes, skipped, oc, lc = solve(routes, store_types)
