...
Demand model: Normal(estimate, sqrt(estimate)), floored at 0, rounded to int.
  - Estimates from 0_5ayush6week-estimated_demand.csv (mean + 0.5 sigma).
  - Std approximated as sqrt(estimate) (Poisson-like for count data).

Traffic model: Per-route Beta-distributed multiplier on travel time.
  ...
  - Unloading time (18 min/pallet) is NOT affected by traffic.

Capacity: 16 pallets per truck. If actual pallets > 16, overflow pallets
  are handled by a wet-lease truck at $1400/2hr block.
...
def get_traffic_dist(day, shift_idx, n_routes):
    """Assign traffic multiplier distribution to each route.
    Routes are split: first half get AM, second half get PM.
    Weekend uses Saturday distributions."""
    is_saturday = day == "Saturday"
    am_key = "Saturday AM" if is_saturday else "Weekday AM"
    pm_key = "Saturday PM" if is_saturday else "Weekday PM"
    ...
    for i in range(n_routes):
        ...
        d = BETA_PARAMS[key]
        dists.append(beta_dist.rvs(d["a"], d["b"]) * (d["high"] - d["low"]) + d["low"])
    return dists


# ── SIMULATION ──
def simulate_day(day, routes, skipped, demand, types, rng):
    """Run one Monte Carlo iteration for a single day. Returns cost breakdown."""
    total_owned = 0.0
    total_leased = 0.0
    total_shed = 0.0
    ...
    for i, r in enumerate(routes):
        ...
        # new duration = (base_travel * traffic_mult) + (actual_pallets * unloading_time)
        est_unload_sec = r["total_pallets"] * UNLOAD_MIN_PER_PALLET * 60
        base_travel = r["total_duration_sec"] - est_unload_sec
        if base_travel < 0:
...
def print_summary(all_results, label):
    ...
    print(f"{'='*70}")
    print(f"{'Day':<12} {'Mean':>10} {'Median':>10} {'Std':>10} {'P5':>10} {'P95':>10} {'OT min':>8} {'Overflow':>8}")
    print("-" * 86)
    weekly_total = 0
    for day in DAYS:
        res = all_results[day]
        ...
        ov = np.mean([r["overflow_routes"] for r in res])
        ...
        print(f"{day:<12} ${np.mean(totals):>8,.0f} ${np.median(totals):>8,.0f} "
              ...
              f"${np.percentile(totals, 95):>8,.0f} {ot:>7.0f}m {ov:>7.1f}")
    print("-" * 86)
